...
export function GetProperties(DatabaseTournament: ITournament) {
  ...
  // تعطيل الإيموتات العادية (1-255) إذا تم تمرير القيمة 0
  if (DisabledEmotes.includes(0)) {
    DisabledEmotes = Array.from({ length: 255 }, (_, i) => i + 1);
  }

  // قائمة بجميع الإيموتات الخاصة/القتالية
  ...
  /**
   * التعديل الجديد:
   * نقوم بتعطيل كل شيء ما عدا "Punch" و "Fire Punch"
   */
  ...
  // تنظيف القائمة من أي أرقام تحكم سالبة لضمان التوافق
  DisabledEmotes = DisabledEmotes.filter((id) => id > 0);
