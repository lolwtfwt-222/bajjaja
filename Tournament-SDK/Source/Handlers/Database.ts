  ...
  try {
    ...
    const components = [
      {
        ...
        components: [
          {
            type: 10,
            content: `@everyone\n\n<:Tournament:1446241761449087088> - A new **Classic Tournament** has been created on **StumblePrix**`,
          },
        ],
        accessory: {
          type: 11,
    ...
    if (tournament.Phases && tournament.Phases.length > 0) {
      tournament.Phases.forEach((phase: any, index: number) => {
        ...
        let phaseContent = `<:StumblePrix:1430984394033598586> - Phase ${index + 1}: **${phaseTypeName}**\n`;
